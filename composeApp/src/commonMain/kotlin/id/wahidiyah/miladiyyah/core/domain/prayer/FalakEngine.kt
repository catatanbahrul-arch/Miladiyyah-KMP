...
data class PrayerTimeResult(val imsak: LocalTime, val subuh: LocalTime, val dzuhur: LocalTime, val ashar: LocalTime, val maghrib: LocalTime, val isya: LocalTime)
...
object FalakEngine {
    ...
    fun calculate(date: LocalDate, lat: Double, lng: Double, timeZone: Double): PrayerTimeResult {
        ...
        return PrayerTimeResult(
            toLocalTime(imsakTime),
            toLocalTime(fajrTime),
            toLocalTime(dzuhurTime),
            toLocalTime(asrTime),
            toLocalTime(maghribTime),
            toLocalTime(ishaTime)
        )
    }
